...
slice() {
    xvfb-run -a orca-slicer --load-settings "$scratch/process.json;$scratch/machine.json" --load-filaments "$scratch/filament.json" --slice 0 --export-3mf "$1.gcode.3mf" --outputdir "$scratch" "$dir/out/$2.stl"
    mv "$scratch/$1.gcode.3mf" "$dir/"
    mv "$scratch/plate_1.gcode" "$scratch/$2.gcode"
    grep -E '^(; model printing time|; total estimated time|; printer_model|; filament used \[g\]|; filament_type|; filament_colour|; enable_support|; layer_height|; curr_bed_type|; brim_type|; brim_width)' "$scratch/$2.gcode"
}
slice batarang-a1-mini batarang
