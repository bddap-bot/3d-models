...
xvfb-run -a orca-slicer --load-settings "$scratch/process.json;$scratch/machine.json" --load-filaments "$scratch/filament.json" --slice 0 --export-3mf batarang-a1-mini.gcode.3mf --outputdir "$dir" "$dir/out/batarang.stl"
mv "$dir/plate_1.gcode" "$scratch/batarang.gcode"
grep -E '^(; model printing time|; total estimated time|; printer_model|; filament used \[g\]|; filament_type|; filament_colour|; enable_support|; layer_height|; curr_bed_type|; brim_type|; brim_width)' "$scratch/batarang.gcode"
