...
Every `model.json` names a `.scad` in its directory and ships the STL and preview PNG of each of its parts under `out/`, written by `./build` and gated by CI.
